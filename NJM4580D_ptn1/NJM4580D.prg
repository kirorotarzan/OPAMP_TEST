...
@@PLAN LOW_PASS_FILTER_TEST;
  SITE_SEQUENCE       = OFF;
  DISABLE_BY_MARK_NO  = NULL;
  REMARK              = AMP_TEST;
  GO_THROUGH          = OFF;

  IO_CLOSE_USER_RELAY("59",CONNECT_MODE,WAIT=1ms);
  WAIT(3ms);

  MIX_HR_DIG_PATH_SET(HR_DIG,INPUT=SINGLE_END,GAIN=1,FILTER=ALL_PASS,ADC_FILTER=ALL_PASS,OFFSET_V=0V,TRIG_SEL=DDS,CH_RLY_ON,TRIG_TYPE=HIGH_ACTIVE);  
  MIX_HR_AWG_PATH_SET(HR_AWG,FILTER=ALL_PASS,GAIN=1,OUTPUT=SINGLE_END,OFFSET_V=0V,CH_RLY_ON);
  WAIT(1ms);
  ...
  //MIX_IM(2000,4000,UP_LIMIT=-5.4dB,DOWN_LIMIT=-5.8dB,FFT_MAG_AMP_CNT=60,LOG_ALIAS=1_IM,LOG_UNIT=dB,BinNo=3);
  ...
  IO_CLOSE_USER_RELAY("59",DISCONNECT_MODE,WAIT=1ms);
  WAIT(3ms);
